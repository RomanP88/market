...
import ru.gb.market.order.services.PayPalService;
    ...
    public ResponseEntity<?> captureOrder(@PathVariable String payPalId) throws IOException {
        ...
        if ("COMPLETED".equals(payPalOrder.status())) {
            long orderId = Long.parseLong(payPalOrder.purchaseUnits().get(0).referenceId());
             Optional<ru.gb.market.order.entities.Order> orderOptional = orderService.findById(orderId);
            ru.gb.market.order.entities.Order tempOrder = orderOptional.orElseThrow(()-> new ResourceNotFoundException("Заказ с индексом " + orderId + " не найден"));
            tempOrder.setStatusCode("Оплачен");
